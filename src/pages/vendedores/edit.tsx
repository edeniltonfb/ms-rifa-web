...
export default function VendedorEditPage() {
    ...
    useEffect(() => {
        showLoader();
        instance.get('/listarcobradoridlabel')
            .then((res) => {
                if (res.data.success) {
    ...
    useEffect(() => {
        if (isEdit) {
            showLoader();
            instance.get(`/buscarvendedor?id=${id}`)
                .then((res) => {
                    if (res.data.success) {
    ...
    const handleChange = (field: keyof VendedorForm, value: any) => {
    ...
    return (
        <div className="max-w-2xl mx-auto">
            <h1 className="text-2xl font-bold mb-4">{isEdit ? 'Editar Vendedor' : 'Novo Vendedor'}</h1>
            ...
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <select
                        className="w-full p-2 rounded border max-h-48 overflow-y-auto bg-white dark:bg-gray-800 dark:text-white"
                        value={form.cobradorId || ''}
                        onChange={(e) => handleChange('cobradorId', parseInt(e.target.value))}
                    >
                    ...
                    {errors.cobradorId && <p className="text-sm text-red-500">{errors.cobradorId}</p>}
                </div>
